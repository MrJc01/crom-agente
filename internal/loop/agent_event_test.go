...
// ═══════════════════════════════════════════════════════════
// Testes de Eventos Estruturados (AgentEvent)
// ═══════════════════════════════════════════════════════════
...
func findEvent(events []AgentEvent, eventType string) *AgentEvent {
	for i := range events {
...
func findEventWithCode(events []AgentEvent, code string) *AgentEvent {
	for i := range events {
		if events[i].Event == "error" {
			if agentErr, ok := events[i].Data["error"].(AgentError); ok {
...
func assertEventExists(t *testing.T, events []AgentEvent, eventType string) {
	t.Helper()
...
func TestAgentEvent_ToolFailureEmitsErrorResult(t *testing.T) {
	...
	if trEvt.Data["error_code"] != ErrToolExecution {
		t.Fatalf("esperado error_code=%s, obteve %v", ErrToolExecution, trEvt.Data["error_code"])
	}
}
...
func TestAgentEvent_ToolNotFoundEmitsError(t *testing.T) {
	...
	agentErr, ok := errEvt.Data["error"].(AgentError)
	if !ok {
		t.Fatalf("esperado AgentError, obteve %T: %v", errEvt.Data["error"], errEvt.Data["error"])
	}
	if agentErr.Code != ErrToolNotFound {
		t.Fatalf("esperado code=%s, obteve %s", ErrToolNotFound, agentErr.Code)
	}
}
...
func TestAgentEvent_ContextCanceledEmitsError(t *testing.T) {
	...
	if agentErr.Code != ErrContextCanceled {
		t.Fatalf("esperado code=%s, obteve %s", ErrContextCanceled, agentErr.Code)
	}
}
...
func TestAgentEvent_ConsecutiveFailuresEmitsFinished(t *testing.T) {
	...
	emptyErr := findEventWithCode(handler.Events, ErrLLMEmptyResponse)
	if emptyErr == nil {
