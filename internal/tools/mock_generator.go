...
func (t *MockGeneratorTool) Execute(ctx context.Context, args json.RawMessage) (Result, error) {
	var input struct {
	...
	if len(interfaces) > 0 {
		...
		if input.OutputPath != "" {
			outFile, err := ValidatePath(t.workspaceRoot, input.OutputPath, t.jail)
			if err != nil {
				return Result{Success: false, Error: err.Error()}, nil
			}
			if err := os.MkdirAll(filepath.Dir(outFile), 0755); err != nil {
				return Result{Success: false, Error: fmt.Sprintf("erro ao criar diretórios: %s", err.Error())}, nil
			}
			if err := os.WriteFile(outFile, []byte(mockCode), 0644); err != nil {
				return Result{Success: false, Error: fmt.Sprintf("erro ao gravar mock: %s", err.Error())}, nil
			}
	...
	if input.GenerateJSON {
		structs, _ := parseGoStructs(targetFile)
		if len(structs) > 0 {
			sb.WriteString("\n\n// === JSON Mock Payloads ===\n\n")
				...
				for _, method := range ifaceType.Methods.List {
					...
					if funcType.Results != nil {
						for _, r := range funcType.Results.List {
							gm.Returns = append(gm.Returns, exprToString(r.Type))
						}
					}
...
func generateJSONMock(s GoStruct) string {
