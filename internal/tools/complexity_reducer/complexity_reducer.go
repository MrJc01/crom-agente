...
func init() {
	...
	if err != nil {
		panic("falha ao carregar metadados de complexity_reducer: " + err.Error())
	}
}
...
func (t *ComplexityReducerTool) ID() string { return metadata.ID }
...
func (t *ComplexityReducerTool) Execute(ctx context.Context, args json.RawMessage) (tools.Result, error) {
	var input struct {
...
func calculateComplexity(fset *token.FileSet, funcDecl *ast.FuncDecl, file string) FuncComplexity {
	...
	if funcDecl.Recv != nil && len(funcDecl.Recv.List) > 0 {
		fc.Receiver = tools.ExprToString(funcDecl.Recv.List[0].Type)
	}
