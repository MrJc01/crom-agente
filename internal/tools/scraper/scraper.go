...
func init() {
	...
	if err != nil {
		panic("falha ao carregar metadados de scraper: " + err.Error())
	}
}
...
func (t *ScraperTool) ID() string { return metadata.ID }
...
func (t *ScraperTool) Execute(ctx context.Context, args json.RawMessage) (tools.Result, error) {
	var input struct {
		URL string `json:"url"`
	}
	...
	if err != nil {
		return tools.Result{Success: false, Error: "URL inválida: " + err.Error()}, nil
	}
	...
	if err != nil {
		return tools.Result{Success: false, Error: fmt.Sprintf("falha ao resolver host %s: %v", host, err)}, nil
	}
	for _, ip := range ips {
	...
	if err != nil {
		return tools.Result{Success: false, Error: "erro ao criar request: " + err.Error()}, nil
	}
	req.Header.Set("User-Agent", "Crom-Agente/0.2.0 (Autonomous Agent; Scraper)")

	resp, err := client.Do(req)
	if err != nil {
		return tools.Result{Success: false, Error: "erro ao disparar requisição: " + err.Error()}, nil
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return tools.Result{Success: false, Error: fmt.Sprintf("resposta do servidor não-200: %s", resp.Status)}, nil
	}
	...
	if err != nil {
		return tools.Result{Success: false, Error: "erro ao parsear HTML: " + err.Error()}, nil
	}
	...
	return tools.Result{Success: true, Data: cleaned}, nil
}
