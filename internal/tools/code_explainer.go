...
func (t *CodeExplainerTool) Execute(ctx context.Context, args json.RawMessage) (Result, error) {
	var input struct {
	...
	if funcCount == 0 {
		if input.FunctionName != "" {
			return Result{Success: false, Error: fmt.Sprintf("função '%s' não encontrada", input.FunctionName)}, nil
		}
...
func explainFunction(sb *strings.Builder, fset *token.FileSet, funcDecl *ast.FuncDecl, level string) {
	...
	if funcDecl.Recv != nil && len(funcDecl.Recv.List) > 0 {
		receiver = fmt.Sprintf(" (método de `%s`)", exprToString(funcDecl.Recv.List[0].Type))
	}
