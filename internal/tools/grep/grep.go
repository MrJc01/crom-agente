...
func init() {
	...
	if err != nil {
		panic("falha ao carregar metadados de grep: " + err.Error())
	}
}
...
func (t *GrepTool) Execute(ctx context.Context, args json.RawMessage) (tools.Result, error) {
	...
	if input.Query == "" {
		return tools.Result{Success: false, Error: "a query de busca não pode estar vazia"}, nil
	}
	...
	if input.IsRegex {
		...
		if err != nil {
			return tools.Result{Success: false, Error: fmt.Sprintf("expressão regular inválida: %s", err.Error())}, nil
		}
	}
	...
	if err != nil {
		return tools.Result{Success: false, Error: fmt.Sprintf("erro durante a varredura do diretório: %s", err.Error())}, nil
	}
	...
	return tools.Result{Success: true, Data: string(jsonData)}, nil
}
