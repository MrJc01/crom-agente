...
func init() {
	...
	if err != nil {
		panic("falha ao carregar metadados de doc_generator: " + err.Error())
	}
}
...
func (t *DocGeneratorTool) ID() string { return metadata.ID }
...
func (t *DocGeneratorTool) Execute(ctx context.Context, args json.RawMessage) (tools.Result, error) {
	var input struct {
	...
	if err != nil {
		return tools.Result{Success: false, Error: fmt.Sprintf("erro ao parsear arquivo: %s", err.Error())}, nil
	}
	...
	for _, decl := range node.Decls {
		...
		if funcDecl.Recv != nil && len(funcDecl.Recv.List) > 0 {
			uf.Receiver = tools.ExprToString(funcDecl.Recv.List[0].Type)
		}
		...
		if funcDecl.Type.Results != nil {
			for _, r := range funcDecl.Type.Results.List {
				uf.Returns = append(uf.Returns, tools.ExprToString(r.Type))
			}
		}
	...
	if len(undocumented) == 0 {
		return tools.Result{Success: true, Data: "Todas as funções exportadas já possuem documentação GoDoc."}, nil
	}
	...
	return tools.Result{Success: true, Data: fmt.Sprintf("✅ Documentação GoDoc inserida para %d funções em %s", len(docs), input.Path)}, nil
}
