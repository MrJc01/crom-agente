...
func init() {
	...
	if err != nil {
		panic("falha ao carregar metadados de delete_file: " + err.Error())
	}
}
...
func (t *DeleteFileTool) Execute(ctx context.Context, args json.RawMessage) (tools.Result, error) {
	var input struct {
	...
	if err != nil {
		return tools.Result{Success: false, Error: "workspace root inválido"}, nil
	}

	// 1. Impedir deletar o próprio diretório raiz do workspace
	if targetFile == absWorkspace {
		return tools.Result{Success: false, Error: "acesso negado: não é permitido deletar o diretório raiz do workspace"}, nil
	}

	// 2. Extrair caminhos relativos para validação
	rel, err := filepath.Rel(absWorkspace, targetFile)
	if err != nil {
		return tools.Result{Success: false, Error: "caminho relativo inválido"}, nil
	}
	...
	for _, part := range parts {
		if part == ".git" || part == ".github" {
			return tools.Result{Success: false, Error: "acesso negado: não é permitido deletar diretórios do Git ou GitHub (.git/.github)"}, nil
		}
	}
	...
	if relClean == ".crom" || strings.HasPrefix(relClean, ".crom"+string(filepath.Separator)) || relClean == ".crom_state.json" {
		return tools.Result{Success: false, Error: "acesso negado: não é permitido deletar arquivos ou pastas de configuração do crom-agente"}, nil
	}

	// 5. Bloquear go.mod principal da raiz
	if relClean == "go.mod" || relClean == "go.sum" {
		return tools.Result{Success: false, Error: "acesso negado: não é permitido deletar o go.mod ou go.sum do projeto raiz"}, nil
	}

	// 6. Verificar se o caminho realmente existe antes de deletar
	if _, err := os.Stat(targetFile); err != nil {
		if os.IsNotExist(err) {
			return tools.Result{Success: false, Error: fmt.Sprintf("arquivo ou diretório não existe: %s", input.Path)}, nil
		}
		return tools.Result{Success: false, Error: err.Error()}, nil
	}
	...
	if err := os.RemoveAll(targetFile); err != nil {
		return tools.Result{Success: false, Error: fmt.Sprintf("erro ao deletar arquivo/diretório: %s", err.Error())}, nil
	}
