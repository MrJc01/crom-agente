...
func init() {
	...
	if err != nil {
		panic("falha ao carregar metadados de memory_leak_scanner: " + err.Error())
	}
}
...
func (t *MemoryLeakScannerTool) ID() string { return metadata.ID }
...
func (t *MemoryLeakScannerTool) Execute(ctx context.Context, args json.RawMessage) (tools.Result, error) {
	var input struct {
	...
	if err != nil {
		return tools.Result{Success: false, Error: fmt.Sprintf("erro na análise: %s", err.Error())}, nil
	}
...
func analyzeFunctionForLeaks(fset *token.FileSet, funcDecl *ast.FuncDecl, file string) []LeakPattern {
	...
	if funcDecl.Type.Params != nil {
		for _, param := range funcDecl.Type.Params.List {
			paramType := tools.ExprToString(param.Type)
