...
func (t *DiffReplaceTool) Description() string {
	return "Substitui um bloco de texto específico por um novo conteúdo dentro de um arquivo do workspace."
}
...
func (t *DiffReplaceTool) Execute(ctx context.Context, args json.RawMessage) (Result, error) {
	var input struct {
	...
	if len(matches) == 0 {
		return Result{Success: false, Error: "target_content não foi encontrado no arquivo"}, nil
	}
	...
	if len(validMatches) == 0 {
		return Result{Success: false, Error: "target_content encontrado, mas fora do intervalo de linhas especificado"}, nil
	}
	...
	if err != nil {
		return Result{Success: false, Error: fmt.Sprintf("erro ao gravar alterações no arquivo: %s", err.Error())}, nil
	}

	return Result{Success: true, Data: fmt.Sprintf("substituição efetuada com sucesso no arquivo %s", input.Path)}, nil
}
