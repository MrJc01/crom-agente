...
func (t *ProxyTool) ID() string { return "proxy" }

func (t *ProxyTool) Description() string {
	return `Inicia, para ou monitora um proxy TCP local temporário para interceptar tráfego e salvar payloads em arquivos de log.
Recomendado para debugar tráfego de rede ou chamadas de API feitas por testes de integração.
Ações suportadas:
- "start": Inicia um proxy local para um host alvo (ex: local_port=8081, target_addr="api.github.com:443")
- "stop": Interrompe um proxy ativo especificando o "proxy_id"
- "list": Lista todos os proxies atualmente ativos`
}
...
func (t *ProxyTool) Execute(ctx context.Context, args json.RawMessage) (Result, error) {
	var input struct {
	...
	case "start":
		if input.TargetAddr == "" {
			return Result{Success: false, Error: "target_addr é obrigatório para iniciar o proxy"}, nil
		}
		return t.startProxy(ctx, input.LocalPort, input.TargetAddr, input.LogFile)
	case "stop":
		if input.ProxyID == "" {
			return Result{Success: false, Error: "proxy_id é obrigatório para parar o proxy"}, nil
		}
		return t.stopProxy(input.ProxyID)
	case "list":
		return t.listProxies()
	default:
		return Result{Success: false, Error: fmt.Sprintf("ação desconhecida: %q", input.Action)}, nil
	}
}

func (t *ProxyTool) startProxy(ctx context.Context, localPort int, targetAddr string, logFile string) (Result, error) {
	// Limitar formato de target_addr
	host, portStr, err := net.SplitHostPort(targetAddr)
	if err != nil {
		return Result{Success: false, Error: "target_addr inválido, deve estar no formato host:porta: " + err.Error()}, nil
	}
	_ = host
	_ = portStr
	...
	if logFile != "" {
		var err error
		logPath, err = ValidatePath(t.workspaceRoot, logFile, t.jail)
		if err != nil {
			return Result{Success: false, Error: "log_file: " + err.Error()}, nil
		}
	} else {
		// Log padrão em .crom/
	...
	if err != nil {
		return Result{Success: false, Error: "falha ao iniciar listener local: " + err.Error()}, nil
	}
...
func (t *ProxyTool) stopProxy(id string) (Result, error) {
	proxiesMu.Lock()
	p, ok := activeProxies[id]
	proxiesMu.Unlock()

	if !ok {
		return Result{Success: false, Error: fmt.Sprintf("proxy com ID '%s' não encontrado", id)}, nil
	}

	p.Listener.Close()
	p.Cancel()

	return Result{Success: true, Data: fmt.Sprintf("Proxy '%s' parado com sucesso. Logs gravados em: %s", id, p.LogPath)}, nil
}

func (t *ProxyTool) listProxies() (Result, error) {
	proxiesMu.RLock()
