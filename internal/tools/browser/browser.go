...
func init() {
	...
	if err != nil {
		panic("falha ao carregar metadados de browser: " + err.Error())
	}
}
...
func (b *BrowserTool) Execute(ctx context.Context, args json.RawMessage) (tools.Result, error) {
	...
	case "navigate":
		if params.URL == "" {
			return tools.Result{Success: false, Error: "URL é obrigatória para a ação 'navigate'"}, nil
		}
		err := page.Navigate(params.URL)
		if err != nil {
			return tools.Result{Success: false, Error: fmt.Sprintf("falha ao navegar para %s: %v", params.URL, err)}, nil
		}
		_ = page.WaitLoad()
		return tools.Result{Success: true, Data: fmt.Sprintf("Navegado com sucesso para %s", params.URL)}, nil

	case "click":
		if params.Selector == "" {
			return tools.Result{Success: false, Error: "selector é obrigatório para a ação 'click'"}, nil
		}
		...
		if err != nil {
			...
			return tools.Result{Success: false, Error: fmt.Sprintf("elemento %q não encontrado (timeout 5s): %v.\nElementos clicáveis disponíveis na página:\n%s", params.Selector, err, sug)}, nil
		}
		err = el.Click(proto.InputMouseButtonLeft, 1)
		searchCancel()
		if err != nil {
			return tools.Result{Success: false, Error: fmt.Sprintf("falha ao clicar no elemento %q: %v", params.Selector, err)}, nil
		}
		...
		return tools.Result{Success: true, Data: fmt.Sprintf("Elemento %q clicado com sucesso. URL atual: %s | Título: %s", params.Selector, newURL, title)}, nil

	case "type":
		if params.Selector == "" || params.Text == "" {
			return tools.Result{Success: false, Error: "selector e text são obrigatórios para a ação 'type'"}, nil
		}
		searchCtx, searchCancel := context.WithTimeout(timeoutCtx, 5*time.Second)
		el, err := page.Context(searchCtx).Element(params.Selector)
		if err != nil {
		...
		if isContentEditable {
			if errFocus := el.Focus(); errFocus != nil {
				searchCancel()
				return tools.Result{Success: false, Error: fmt.Sprintf("falha ao focar elemento editável %q: %v", params.Selector, errFocus)}, nil
			}
		...
		if err != nil {
			return tools.Result{Success: false, Error: fmt.Sprintf("falha ao digitar no elemento %q: %v", params.Selector, err)}, nil
		}
		...
		return tools.Result{Success: true, Data: fmt.Sprintf("Texto digitado com sucesso no elemento %q. URL atual: %s | Título: %s", params.Selector, newURL, title)}, nil

	case "screenshot":
		if params.URL != "" {
			err := page.Navigate(params.URL)
			if err != nil {
				return tools.Result{Success: false, Error: fmt.Sprintf("falha ao navegar para %s antes de capturar screenshot: %v", params.URL, err)}, nil
			}
			_ = page.WaitLoad()
		...
		if err != nil {
			return tools.Result{Success: false, Error: fmt.Sprintf("falha ao capturar screenshot: %v", err)}, nil
		}
		b64 := base64.StdEncoding.EncodeToString(imgBytes)
		if params.Path != "" {
			targetFile, err := tools.ValidatePath(b.workspacePath, params.Path, false)
			if err != nil {
	...
	case "get_html":
		html, err := page.HTML()
		if err != nil {
			return tools.Result{Success: false, Error: fmt.Sprintf("falha ao extrair HTML: %v", err)}, nil
		}
		return tools.Result{Success: true, Data: html}, nil

	default:
		return tools.Result{Success: false, Error: fmt.Sprintf("ação desconhecida: %s", params.Action)}, nil
	}
}
